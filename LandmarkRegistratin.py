...
class LandmarkRegistrationWidget:
  ...
  def onHybridApply(self):
    ...
    #was: reg.SetMovingId(getNode(img_moving).GetID())
    #reg.SetInputXfId(getNode(xf_in).GetID())
    reg.SetOutputImageName("OutputImg")

#   does not work: type error, needs string
    #transformed = self.volumeSelectors['Transformed'].currentNode()
    #reg.SetOutputImageName(str(self.volumeSelectors['Transformed'].currentNode()))

    # Set landmarks from Slicer (not required)
    #reg.AddLandmark(getNode("fix-landmarks").GetID(), "fixed")
    #reg.AddLandmark(getNode("mov-landmarks").GetID(), "moving")
